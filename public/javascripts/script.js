...
let c4;
let  filter_size;
let  filter_priceAbove;
let filter_priceBelow;
...
retakeBtn.addEventListener("click", async () => {
  ...
  function findSimilarjeans(color1, color2, color3, color4, contrastColor, threshold) {
    ...
    if (jeans_image_filtered.length === 0) {
...
function findSimilarUppers(color1, color2, color3, color4, contrastColor, threshold) {
  ...
  if (upper_image_filtered.length === 0) {
    document.getElementById("message").style.display="block";
...
function findSimilarLowers(color1, color2, color3, color4, contrastColor, threshold) {
  ...
  if (lower_image_filtered.length === 0) {
    document.getElementById("message").style.display="block";
...
// Update displayed value for Price Above
function updateAbovePrice(value) {
  document.getElementById('price-above-value').innerText = value;
}

// Update displayed value for Price Below
function updateBelowPrice(value) {
  document.getElementById('price-below-value').innerText = value;
}

// Function to handle the Done button click
function setFilterValues() {
  filter_size=document.getElementById('size').value;
  filter_priceAbove = document.getElementById('price-above').value;
  filter_priceBelow = document.getElementById('price-below').value;
  const filterInfo = document.getElementById("filter-info");
  filterInfo.style.display = 'none'; // Hide filter info
if(filter_size){
  filterInfo.innerText=`Filter:Price Above: ${filter_priceAbove}, Price Below: ${filter_priceBelow}, size: ${filter_size}`
  filterInfo.style.display = 'block'; // Show filter info
...
function clearFilters() {
  document.getElementById("message").style.display="none";
  // Reset filter values
  filter_size = false; // Assuming filter_size is a global variable
  filter_priceAbove = false; // Assuming filter_priceAbove is a global variable
  filter_priceBelow = false; // Assuming filter_priceBelow is a global variable
  // Hide filter info and clear button
  document.getElementById("filter-info").style.display = 'none'; // Hide filter info
